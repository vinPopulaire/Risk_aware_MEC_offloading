...
# repeat if any of the values is negative since we want kn to be positive
kn = 1.2 + np.random.uniform(-1,1, size=N) * 3
while np.any(kn <= 0):
    kn = 1.2 + np.random.uniform(-1,1, size=N) * 3
...
for repetition in range(1):
    ...
    if params["SAVE_RESULTS"] == True:
        outfile = 'saved_runs/results/individual/' + "hetero_kn_" + str(np.round(np.mean(kn), 3)) + '_' + str(repetition)

        with open(outfile, 'wb') as fp:
            dill.dump(results, fp)

kn = np.mean(kn) * np.ones(N)
params["kn"] = kn

print("Mean kn: " + str(np.round(np.mean(kn), 3)))
...
for repetition in range(1):
    ...
    if params["SAVE_RESULTS"] == True:
        outfile = 'saved_runs/results/individual/' + "homo_kn_" + str(np.round(np.mean(kn), 3)) + '_' + str(repetition)
...
infile = 'saved_runs/results/individual/' + "homo_kn_" + str(np.round(np.mean(kn), 3)) + '_' + str(repetition)
...
infile = 'saved_runs/results/individual/' + "hetero_kn_" + str(np.round(np.mean(kn), 3)) + '_' + str(repetition)
with open(infile, 'rb') as in_strm:
    kati = dill.load(in_strm)
...
axs[0,1].ticklabel_format(style='sci', axis='y', scilimits=(7,7))
...
axs[1,0].ticklabel_format(style='sci', axis='y', scilimits=(7,7))
...
path_name = "hetero_kn_" + str(np.round(np.mean(kn),3)) + "_utility_bars"
